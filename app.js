...
let win, custominput, vcodec, acodec, prefix, present, crf, threads, ignore
...
async function processVideo(array) {
	...
	tempy = tempy + '"' + ffmpegloc() + '" -crf ' + crf + ' -preset ' + present + ' -i ' + '"' + item[0] + '" ' + (custominput !== '' ?custominput : '') + ' -y ' + (ignore == 'on' ? '' : '-filter_complex "subtitles=' + "'" + escaped_path_output.split(':/').join('\\:/') + "'" + '" ') + '-c:a lib' + acodec + ' -c:v lib' + vcodec + ' -threads ' + threads + ' "' + outputloc + '"' + os.EOL
    }
